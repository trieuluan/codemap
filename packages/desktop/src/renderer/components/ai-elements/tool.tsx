...
export const Tool = ({ className, ...props }: ToolProps) => (
  <Collapsible
    className={cn("tool-inline not-prose w-full", className)}
    {...props}
  />
);
...
export const ToolHeader = ({
  ...
  <CollapsibleTrigger
    className={cn(
      "tool-trigger flex w-full items-center justify-between gap-4 p-3",
      className
    )}
    {...props}
  >
    <div className="flex items-center gap-2">
      ...
      <span className="font-medium text-sm">
        {title ?? type.split("-").slice(1).join("-").replace(/-/g, " ")}
      </span>
