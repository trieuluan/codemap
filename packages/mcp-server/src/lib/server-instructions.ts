...
|---|---|
| New broad task | explore_task → get_files → get_file |
| "Which files to read?" | find_related_files |
| Symbol / keyword lookup | search_codebase or find_usages |
| Impact analysis | find_callers / find_usages |
| Read specific symbol body | get_file(include=["symbols"], symbol_names=[...]) |
| Survey file structure | get_file(include=["outline"]) |
| After edits | get_working_diff → trigger_reimport |
...
- Do NOT declare a task complete without reviewing get_working_diff.
