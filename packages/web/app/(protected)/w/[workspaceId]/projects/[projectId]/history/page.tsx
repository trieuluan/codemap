        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Compare snapshots of <span className="font-medium text-foreground">{project.name}</span> over time.
            </p>
          </div>
          <Button variant="outline" size="sm" asChild>
            <Link href={`/w/${workspaceId}/projects/${project.id}`}><ChevronLeft className="size-4" />Back to project</Link>
          </Button>
        </div>
        <ProjectHistoryView projectId={project.id} initialImports={firstPage.data} />
      </div>
    );
  } catch (error) {
